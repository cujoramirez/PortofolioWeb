...
const SplashCursorComponent = ({
  ...
  BACK_COLOR = { r: 0.1, g: 0.05, b: 0.25 },
  TRANSPARENT = true,
}: SplashCursorProps) => {
  ...
  useEffect(() => {
    ...
    const getWebGLContext = (target: HTMLCanvasElement) => {
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      if (!gl) {
        gl = (target.getContext('webgl', params) || target.getContext('experimental-webgl', params)) as WebGL2RenderingContext | null;
      }

      if (!gl) {
        return { gl: null, ext: null } as const;
      }
      ...
      const getSupportedFormat = (internalFormat: number, format: number, type: number) => {
        const supported = supportRenderTextureFormat(gl, internalFormat, format, type);
        if (!supported) {
      ...
      if (isWebGL2) {
        const gl2 = gl as WebGL2RenderingContext;
        formatRGBA = getSupportedFormat(gl2.RGBA16F, gl.RGBA, halfFloatTexType);
    ...
    const resizeFBO = (target: FBO, w: number, h: number, internalFormat: number, format: number, type: number, param: number) => {
    ...
    const resizeDoubleFBO = (target: DoubleFBO, w: number, h: number, internalFormat: number, format: number, type: number, param: number) => {
    ...
    const generateColor = (): ColorRGB => {
      // Generate colors in the indigo/cyan/purple range to match the theme
      const colorChoice = Math.random();
      let color: ColorRGB;
      
      if (colorChoice < 0.33) {
        // Indigo range (hue ~240-260)
        color = HSVtoRGB(0.66 + Math.random() * 0.06, 0.85 + Math.random() * 0.15, 0.9 + Math.random() * 0.1);
      } else if (colorChoice < 0.66) {
        // Cyan/Sky range (hue ~180-200)
        color = HSVtoRGB(0.5 + Math.random() * 0.06, 0.8 + Math.random() * 0.2, 0.85 + Math.random() * 0.15);
      } else {
        // Purple/Violet range (hue ~270-290)
        color = HSVtoRGB(0.75 + Math.random() * 0.06, 0.7 + Math.random() * 0.3, 0.8 + Math.random() * 0.2);
      }
      
      // Adjust intensity for better visibility against dark background
      color.r *= 0.35;
      color.g *= 0.35;
      color.b *= 0.35;
      return color;
    ...
    const handleFirstTouchStart = (e: TouchEvent) => {
      ...
      for (let i = 0; i < e.targetTouches.length; i++) {
        ...
        updateFrame();
        updatePointerDownData(pointer, e.targetTouches[i].identifier, posX, posY);
      }
  ...
  }, [CAPTURE_RESOLUTION, COLOR_UPDATE_SPEED, CURL, DENSITY_DISSIPATION, DYE_RESOLUTION, PRESSURE, PRESSURE_ITERATIONS, SIM_RESOLUTION, SPLAT_FORCE, SPLAT_RADIUS, TRANSPARENT, VELOCITY_DISSIPATION]);

  return (
    <div className="fixed inset-0 -z-10 pointer-events-none" style={{ width: '100vw', height: '100vh', left: 0, top: 0 }}>
      <canvas ref={canvasRef} className="w-full h-full" style={{ display: 'block', width: '100%', height: '100%' }} />
    </div>
  );
};

export default memo(SplashCursorComponent);

...
type RGBColor = Pick<ColorComponents, 'r' | 'g' | 'b'>;

type AnyRecord = Record<string, any>;

interface TechnologyColor {
  index: number;
  color: RGBColor;
...
interface PerformanceState {
  ...
  initialQuality: 'low' | 'medium' | 'high' | null;
  lowFpsFrames: number;
...
const createColorComponents = (r: number, g: number, b: number, a: number): ColorComponents => ({
  r: clampChannel(r),
  g: clampChannel(g),
  b: clampChannel(b),
  a: clampAlpha(a),
...
const cloneColorComponents = (color: ColorComponents): ColorComponents => ({
  r: color.r,
  g: color.g,
  b: color.b,
  a: color.a,
...
const colorToString = (color: ColorComponents, alphaOverride?: number): string => {
...
const applyColor = (target: AnyRecord, colorComponents: ColorComponents): void => {
  target.colorComponents = colorComponents;
  target.color = colorToString(colorComponents);
};

const applyBaseColor = (target: AnyRecord, colorComponents: ColorComponents): void => {
...
const LightEffectsComponent = ({ hoveredTech, hoveredTechRef }: LightEffectsProps) => {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const dimensionsRef = useRef<DimensionsState>({ width: 0, height: 0 });
  const techElementsRef = useRef<HTMLElement[]>([]);
  const isNestHubMaxRef = useRef(false);
  const animationRef = useRef<number | null>(null);
  const particlesRef = useRef<AnyRecord[]>([]);
  const orbitalsRef = useRef<AnyRecord[]>([]);
  const molecularStructuresRef = useRef<AnyRecord[]>([]);
  const strayParticlesRef = useRef<AnyRecord[]>([]);
  const mousePositionRef = useRef<MousePosition>({ x: null, y: null });
  const scrollStateRef = useRef<ScrollState>({ isScrolling: false, lastEventTime: 0 });
  const activeColorSchemeRef = useRef<RGBColor | null>(null);
  ...
  const techColorsMap = useRef<TechnologyColor[]>(
...
export default memo(LightEffectsComponent);

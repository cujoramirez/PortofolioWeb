    ...
    >
      ...
      <Container maxWidth="xl" sx={{ position: 'relative', zIndex: 10 }}>
        <motion.div
          ...
          style={{ textAlign: 'center', marginBottom: 48, paddingTop: 32, paddingBottom: 32, y: shouldUseScrollTrigger ? titleY : 0, opacity: 1 }}
        >
          ...
          <Box sx={{ position: 'relative', display: 'flex', justifyContent: 'center', mt: 2 }}>
            <Box sx={{ width: 200, height: 4, background: 'linear-gradient(90deg, transparent 0%, #6366f1 20%, #22d3ee 50%, #8b5cf6 80%, transparent 100%)', borderRadius: 2, opacity: 0.8 }} />
          </Box>
        </motion.div>
              ...
              {categories.map((category, index) => {
                ...
                return (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.95 }}
                    style={{ flex: { xs: '1 1 calc(50% - 6px)', sm: '0 1 auto' }, minWidth: { xs: 'calc(50% - 6px)', sm: '100px' }, maxWidth: { xs: 'calc(50% - 6px)', sm: '140px' } }}
                  >
                    <Paper
                      ...
                      sx={{
                        p: { xs: 1.5, sm: 2 },
                        borderRadius: 3,
                        background: isSelected ? `linear-gradient(135deg, ${category.color}40, ${category.color}20)` : 'linear-gradient(135deg, rgba(30, 41, 59, 0.8), rgba(15, 23, 42, 0.9))',
                        ...
                        position: 'relative',
                        overflow: 'hidden',
                        '&::before': isSelected && performanceTier !== 'low' && !isHandheld ? { content: '""', position: 'absolute', inset: 0, background: `linear-gradient(45deg, transparent 30%, ${category.color}20 50%, transparent 70%)`, animation: 'categoryShimmer 2s ease-in-out infinite', zIndex: 0 } : {},
                        '&:hover': { transform: 'translateY(-2px)', background: `linear-gradient(135deg, ${category.color}30, ${category.color}15)`, boxShadow: `0 8px 32px ${category.color}30` }
                      }}
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: { xs: 1, sm: 2 }, flexDirection: { xs: 'column', sm: 'row' }, position: 'relative', zIndex: 1 }}>
                        <IconComponent sx={{ color: isSelected ? category.color : 'rgba(255, 255, 255, 0.7)', fontSize: { xs: 24, sm: 28 }, transition: 'all 0.3s ease', filter: isSelected ? `drop-shadow(0 0 8px ${category.color}50)` : 'none' }} />
                        <Typography variant="h6" sx={{ color: isSelected ? 'white' : 'rgba(255, 255, 255, 0.8)', fontWeight: isSelected ? 700 : 500, fontSize: { xs: '0.8rem', sm: '1.25rem' }, transition: 'all 0.3s ease', textAlign: 'center', textShadow: isSelected ? `0 0 8px ${category.color}50` : 'none' }}>
                          {category.name}
                        </Typography>
                      </Box>
                    </Paper>
